...
$(function() {
    ...
    $(document).on('change', '.ecb_file_selector select', function (e) {
        ...
        var file_name = $(this).val(),
            $file_thumb = $(this).closest('.ecb_file_selector').find('.ecb_file_selector_preview');
    ...
    $(document).on('change', '.ecb_file_picker .cmsfp_elem', function (e) {
        e.preventDefault();
        var file_name = $(this).val(),
            $file_picker = $(this).closest('.ecb_file_picker'),
    ...
    function ECB2InitialiseSubFields( $wrapper, blockName, row ) {
        $wrapper.find('.repeater-field').each( function() {
            ...
            if ( $(this).hasClass('ecb2-file-picker-template') ) {   // setup 
                $(this).removeClass('ecb2-file-picker-template').addClass('ecb2-file-picker');
                initailiseFilePicker( $(this) );
            }
   ...
   $('ul.sortable-ecb2-list').each(function() {
    ...
    /* cmsms file_picker */
    function initailiseFilePicker ($filepicker) {
    ...
    $('.ecb2-dropzone.sortable').sortable({
        ...
        stop: function(e,ui) {
            let max_files = $(this).data('dropzone-max-files'),
        ...
        $(this).dropzone({
            ...
            init: function() {
                ...
                this.on("removedfile", function(file) {
                    let removed_row = $(file.previewElement).data('row');
                ...
                json_values.forEach((saved_filename, i) => {
                    if (saved_filename!='') {
    ...
    $(document).on('click', '.dz-edit', function(e) {
        e.preventDefault();
        let blockName = $(this).closest('.ecb2-dropzone').data('blockName'),
