...
# END_LICENSE
#-------------------------------------------------------------------------
# deprecated marked for removal in 2.0
# TODO remove

if( !defined('CMS_VERSION') ) exit;

/*  **  */
$item_id = isset($_REQUEST['itemid']) ? (int)$_REQUEST['itemid'] : '';
$cat_id = isset($_REQUEST['category']) ? (int)$_REQUEST['category'] : '';

die( \LISE\api::GenerateSlug( '', $item_id, $cat_id, $this->GetName() ) );
/*  **  */
...
//
//if( strstr($template, '{$item_category') )
//{
//  $ids = array($cat_id);
//  $cats = LISECategoryOperations::GetCategoryNameFromId($this, $ids);
//  if(count($cats))
//    $category_name = munge_string_to_url( trim($cats[0]) );
//}
//
//if( empty($category_name) ) $category_name = 'no-category';
//
//$smarty->assign('item_title', munge_string_to_url( trim($_REQUEST['title']) ) );
//$smarty->assign('item_category',  $category_name);
