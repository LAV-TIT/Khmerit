<fieldset>
  ...
  <div class="pageoverflow">
    <p class="pagetext">{$fielddef->ModLang('prompt_include_items')}:</p>
    <p class="pageinput">
	    {html_options name="`$actionid`custom_input[include_items]" options=$fielddef->IncludeItems() selected=$fielddef->GetOptionValue('include_items')}
    </p>
    <p class="pagetext"><div class="information">{$fielddef->ModLang('fielddef_identifier_help')}</div></p>
  </div>
  <hr>
